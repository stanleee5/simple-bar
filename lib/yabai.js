...
export async function getWindows() {
  ...
  const windows = Utils.parseJson(cleanedJson);
  // Local patch: yabai loses window focus when switching macOS native tabs
  // (e.g. Ghostty) — no window reports has-focus and no window_focused event
  // fires. Infer focus as the visible window of the frontmost app on the
  // focused space so the process widget keeps highlighting the current tab.
  if (Array.isArray(windows) && !windows.some((w) => w["has-focus"])) {
    try {
      const [frontApp, spaceJson] = await Promise.all([
        Uebersicht.run(
          `/usr/bin/lsappinfo info -only name $(/usr/bin/lsappinfo front) | /usr/bin/cut -d'"' -f4`,
        ),
        Uebersicht.run(`${yabaiPath} -m query --spaces --space`),
      ]);
      const focusedSpace = Utils.parseJson(spaceJson)?.index;
      const app = frontApp.trim();
      const candidates = windows.filter(
        (w) => w.app === app && w["is-visible"] && !w["is-minimized"],
      );
      const candidate =
        candidates.find((w) => w.space === focusedSpace) ?? candidates[0];
      if (candidate) candidate["has-focus"] = true;
    } catch (e) {
      // eslint-disable-next-line no-console
      console.warn("Failed to infer focused window:", e);
    }
  }
  return windows;
}
